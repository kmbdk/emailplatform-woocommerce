...
final class WC_Emailplatform_Compatibility {
	...
	function add_hooks() {
		...
		add_filter( 'wc_emailplatform/admin/notices', array( $this, 'insert_admin_notices' ) );
	...
	/**
	 * @return WC_Emailplatform_Compatibility
	 */
	...
	public static function check_php() {
		if ( false === version_compare( phpversion(), WC_Emailplatform_MIN_PHP_VERSION , '>=' ) ) {

			self::$notices['php_version'] = array(
				'class' => 'notice-error',
				'message' => sprintf( __( "%sWooCommerce eMailPlatform requires PHP Version %s or newer.%s \n\nYou're using Version %s. Please ask your host to upgrade your server's PHP.", 'emailplatform-woocommerce' ), '<h3>', WC_Emailplatform_MIN_PHP_VERSION, "</h3>\n\n", '<span style="font-family: Consolas, Courier, monospace;">'.phpversion().'</span>' )
			);
	...
	public static function check_wordpress() {
		...
		if ( version_compare( $wp_version, WC_Emailplatform_MIN_WP_VERSION ) <= 0 ) {

			self::$notices['wp_version'] = array(
				'class' => 'notice-error',
				'message' => sprintf( __( "%sWooCommerce eMailPlatform requires WordPress %s or newer.%s \n\nYou're using Version %s. Please upgrade your WordPress installation.", 'emailplatform-woocommerce' ), '<h3>', WC_Emailplatform_MIN_WP_VERSION, "</h3>\n\n", '<span style="font-family: Consolas, Courier, monospace;">'.$wp_version.'</span>' )
			);
	 ...
	 *
	 * @access public
	 * @return boolean True: checks have been passed; WooCommerce Emailplatform is fine to run; False: checks have failed, don't continue loading
	 */
	public static function check_woocommerce() {
		...
		if ( true === $woocommerce_is_active ) {
			...
			if ( true === version_compare( $woocommerce_version, WC_Emailplatform_MIN_WC_VERSION, ">=" ) ) {
				return true;
			}
			...
			self::$notices['wc_version'] = array(
				'class' => 'notice-error',
				'message' => sprintf( __( "%sWooCommerce eMailPlatform requires WooCommerce Version %s or newer.%s You're using Version %s. %sUpdate WooCommerce%s to use the WooCommerce eMailPlatform plugin.", 'emailplatform-woocommerce' ), '<h3>', WC_Emailplatform_MIN_WC_VERSION, "</h3>\n\n", '<span style="font-family: Consolas, Courier, monospace;">'.$woocommerce_version.'</span>', $button, '</strong></a>' )
			);
		...
		switch ( $wc_status ) {
			case 'inactive':
				...
				self::$notices['wc_inactive'] = array( 'class' => 'error', 'message' => sprintf( __( '%sWooCommerce eMailPlatform requires WooCommerce to be active. %sActivate WooCommerce%s to use the WooCommerce Emailplatform plugin.', 'emailplatform-woocommerce' ), '<h3>', "</h3>\n\n". $button, '</a></strong>' ) );
				break;
			default:
				...
				self::$notices['wc_installed'] = array( 'class' => 'error', 'message' => sprintf( __( '%sWooCommerce eMailPlatform requires WooCommerce to be installed in order to run properly. %sInstall WooCommerce%s to use the WooCommerce eMailPlatform plugin.', 'emailplatform-woocommerce' ), '<h3>', "</h3>\n\n". $button, '</a></strong>') );
				break;
		}
